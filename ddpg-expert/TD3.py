...
class TD3:
    def __init__(self, lr, state_dim, action_dim, max_action, iql_policy):
    ...
    def select_action(self, state, explore=True):
        if explore and random.random() < self.iql_clip:
            state = torch.FloatTensor(state.reshape(1, -1)).to(device)
            action = self.iql_policy.policy.act(state)
    ...
    def update(self, replay_buffer, n_iter, batch_size, gamma, polyak, policy_noise, noise_clip, policy_delay):
        self.iql_clip = 0.8*(1-self.episode/3000)
        self.episode += 1
