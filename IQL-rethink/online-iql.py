...
from src.util import return_range, set_seed, Log, sample_batch, torchify, evaluate_policy, collect_traj, RolloutBuffer, sample_batch_online
...
def main(args):
    ...
    iql = ImplicitQLearning(
        ...
        nvf=ValueFunction(obs_dim, hidden_dim=args.hidden_dim, n_hidden=args.n_hidden),
        policy=policy,
    ...
    if len(args.load_policy) > 0:
        iql.load_state_dict(torch.load(args.load_policy))
        #policy.load_state_dict(torch.load(args.load_policy, strict=False))

    buffer = RolloutBuffer()
    for step in range(300):
        collect_traj(env, policy, args.max_episode_steps, buffer)

    # 最多10000轮次， 每次一个新轨迹，200次保存一次
    for step in trange(args.n_steps):
        collect_traj(env, policy, args.max_episode_steps, buffer)
        iql.update(**sample_batch_online(buffer.dataset, args.batch_size))
        if (step+1) % args.eval_period == 0:
            eval_policy()
            torch.save(iql.state_dict(), log.dir / f'policy-{step }.pt')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval-period', type=int, default=200)
